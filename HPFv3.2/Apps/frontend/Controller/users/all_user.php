<?php
	switch(Input::post("action")){
		case "add":
			...
			$key = F::Uniqkey("USER_");
			$data = [
				"user_name"				=> Input::post("name", true, ["uppercase" => true]),
				...
				"user_sector"			=> Input::post("sector"),
				"user_position"			=> Input::post("position")
			];
			...
			if(count($a) > 0){
				...
				new Alert("success","User information has been added.");
			}else{
		...
		case "edit":
			...
			if(count($t) > 0){
				...
				new Alert("success","User information has bee updated.");
			}else{
