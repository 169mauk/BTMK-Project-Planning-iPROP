					...
					<table class="table table-hover table-fluid table-bordered dataTable">
						...
						?>
							<tr>
								...
								<td class="text-center">
									<a title="Edit Info" href="<?= F::URLParams() ?>/edit/<?= $c->cl_id?>" class="btn btn-sm btn-warning">
										<span class="icon-plus2"></span> Edit
									</a>
									<a title="Delete Info" href="<?= F::URLParams() ?>/delete/<?= $c->cl_id ?>" class="btn btn-sm btn-danger">
							...
							if(count($comp) > 0){
								...
								<form action="" method="POST" enctype="multipart/form-data">
									<div class="row">
										...
										<div class="col-md-4">
											Title:
											<input type="text" class="form-control" placeholder="Title" name="c_title" value="<?= $comp->cl_title ?>"/><br />
										</div>
										<div class="col-md-4">
											Company:
											<select class="form-control selectpicker" data-live-search="true" name="c_company">
											<?php
												foreach(companies::list() as $cc){
												?>
