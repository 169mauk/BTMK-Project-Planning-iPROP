					...
					<table class="table table-hover table-fluid table-bordered dataTable">
						...
						?>
							<tr>
								...
								<td class="text-right">
									<a title="Edit Info" class="btn btn-sm btn-primary" href="<?= F::URLParams() ?>/edit/<?= $c->user_id ?>"><span class="icon-edit"></span></a>
									<a title="Delete Info" class="btn btn-sm btn-danger" href="<?= F::URLParams() ?>/delete/<?= $c->user_id ?>"><span class="icon-trash"></span></a>
								</td>
							</tr>
						<?php
							}
				...
				<div class="card-body">
					<form action="" method="POST">
						<div class="row">
							...
							<div class="col-md-3">
								Position:
								<select class="form-control selectpicker" data-live-search="true" name="position" >
								<?php
									foreach(positions::list() as $role){
									?>
										<option value="<?= $role->p_id ?>">
											<?= $role->p_name ?>
										</option>
									<?php
										}
									...
									<div class="row">
										...
										<div class="col-md-3">
											...
											<?php
												...
												foreach($r as $key => $role){
												?>
													<option value="<?= $key ?>" <?= $t->user_role == $key ? "selected" : "" ?>>
														<?= $role ?>
													</option>
												<?php
												}
										...
										<div class="col-md-3">
											...
											<?php
												foreach(positions::list() as $role){
												?>
													<option value="<?= $role->p_id ?>" <?= $t->user_position == $role->p_id ? "selected" : "" ?>>
														<?= $role->p_name ?>
													</option>
												<?php
													}
