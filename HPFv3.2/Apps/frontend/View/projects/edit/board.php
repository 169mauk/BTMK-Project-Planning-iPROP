...
<?php
	foreach (settings::getBy(["s_key" => "task_status"]) as $statusRow) {
		$taskResult = tasks::list(["where" => "t_group = (SELECT tg_id FROM task_group WHERE tg_project = ". url::get(3) ." ORDER BY tg_id DESC LIMIT 1) AND t_kstatus = " . $statusRow->s_value]);
?>
